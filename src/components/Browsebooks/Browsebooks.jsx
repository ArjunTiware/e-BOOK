...
const Browsebooks = () => {
  ...
  useEffect(() => (setFilterbook(books), setfound(false)), [books, value]);

  function handleClick() {
    let n = value.trim().toLocaleLowerCase();
    const filterdbook = books.filter(
      (e) =>
        e.title.toLowerCase() == n.toLocaleLowerCase() ||
        e.author.toLowerCase() == n.toLowerCase()
    );
  ...
  return (
    <>
      ...
      <h1 className="text-3xl font-semibold text-green-700 text-center">
        {books.length > 0 ? `popular ${category} Books` : "Books not found"}{" "}
      </h1>
      <br />
      <section className="container flex flex-wrap gap-5 justify-center">
